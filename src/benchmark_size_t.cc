...
class Benchmark {
    ...
    void benchmark_compression(hshm::Compressor &comp, hshm::Distribution &dist, size_t raw_size) {
        ...
        std::vector<size_t> raw(raw_size);
        std::vector<size_t> compressed(raw_size);
        std::vector<size_t> decompressed(raw_size);
        size_t raw_size_bytes = raw_size * sizeof(size_t);
...
int main(int argc, char** argv) {
    ...
    */
    ...
    // Usage:
    // ./benchmark_size_t count algo dist size [seed] [shape]
    ...
    if (argc < 5) {
        HIPRINT("Usage: ./benchmark_size_t count algo dist size [seed] [shape]\n");
        return 1;
    }
