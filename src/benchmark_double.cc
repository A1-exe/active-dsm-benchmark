...
class Benchmark {
    ...
    void benchmark_compression(hshm::Compressor &comp, hshm::Distribution &dist, size_t raw_size) {
        ...
        std::vector<double> raw(raw_size);
        std::vector<double> compressed(raw_size);
        std::vector<double> decompressed(raw_size);
        size_t raw_size_bytes = raw_size * sizeof(double);
...
int main(int argc, char** argv) {
    ...
    */
    ...
    // Usage:
    // ./benchmark_double count algo dist size [seed] [shape]
    ...
    if (argc < 5) {
        HIPRINT("Usage: ./benchmark_double count algo dist size [seed] [shape]\n");
        return 1;
    }
